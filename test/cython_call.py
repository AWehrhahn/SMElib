...
def check_data_files_exist():
    ...
    """
    names = dll.GetDataFiles()
    directory = dll.GetLibraryPath()
    for name in names:
        n = join(directory, name)
        if not exists(n):
...
# atomic = np.zeros((2, 8))
# linelist = {"species": species, "atomic": atomic}
# resp = cywrapper.InputLineList(linelist)
# print(resp)
...
datadir = realpath(join(dirname(__file__), "../share/libsme"))
